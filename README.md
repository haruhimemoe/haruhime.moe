...
The logos, icons, link preview, palette and repo banners in `public/brand/` and `src/app/` are generated by [@haruhimemoe/brand](https://github.com/haruhimemoe/brand). [CONTRIBUTING.md](./CONTRIBUTING.md#brand-files) shows how to regenerate them. The swatch hex values on `/brand` come from the generated `public/brand/haruhime-palette.json`, and a test checks them against the [@haruhimemoe/ui](https://github.com/haruhimemoe/ui) theme, so colors change upstream, not in `brand.ts`.
