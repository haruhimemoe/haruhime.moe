# haruhime.moe

The home page for haruhime's osu! tournament tools: [packs](https://packs.haruhime.moe), with pools and sheets on the way. It also has a thanks list, the brand kit, contact details and a disclaimer.

Live at https://haruhime.moe.

Found a problem with the site? Open an issue, or see [CONTRIBUTING.md](./CONTRIBUTING.md) to run it yourself. Security reports go to contact@haruhime.moe ([SECURITY.md](./SECURITY.md)).

## License

MIT, see [LICENSE](./LICENSE). osu! is a trademark of ppy Pty Ltd; this site isn't affiliated with or endorsed by ppy.
